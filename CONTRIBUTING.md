...
6. Use the existing movement slugs where possible (see `data/movements.json`). If your entry needs a **new** movement, register it in `scripts/build_movements.py` (add the slug to the `GROUPS` map) and run `python3 scripts/build_movements.py` — otherwise validation will reject the unknown movement.
7. Run `python3 scripts/validate.py` to check your entry against the schema (also verifies `source_notes` is present, `id` matches the filename, ids are unique, and every movement exists in `movements.json`). Requires `pip install jsonschema`.
8. Run `python3 scripts/build_index.py` to regenerate the index and your entry's `.md` twin.

CI runs these on every PR (`.github/workflows/validate.yml`) — a PR that fails validation won't be merged, so running them locally first saves a round-trip.
